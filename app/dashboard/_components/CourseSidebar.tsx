                           ...
                           <Button variant="outline" className="w-full p-3 h-auto flex items-center gap-2">
                               ...
                               <div className="flex-1 text-left min-w-0">
                                  <p className="font-semibold text-sm truncate text-foreground">
                                   ...
                                   <p className="text-[10px] text-muted-foreground font-medium truncate">{chuong.baiHocs.length} bài học</p>
                               </div>
                           </Button>
                          </CollapsibleTrigger>
                          <CollapsibleContent className="mt-3 pl-6 border-l-2 space-y-3">
                           {chuong.baiHocs.map((baiHoc)=>(
                               <LessonItem completed={baiHoc.tienTrinhHocs.find((tienTrinh)=>tienTrinh.idBaiHoc === baiHoc.id)?.hoanThanh || false} key={baiHoc.id} lesson={baiHoc } slug={course.duongDan} isActive={currentLessonId === baiHoc.id}></LessonItem>
                           ))}
                          </CollapsibleContent>
                    </Collapsible>
                ))}
