                           ...
                           <Button variant="outline" className="w-full p-3 h-auto flex items-center gap-2">
                               ...
                               <div className="flex-1 text-left min-w-0">
                                  <p className="font-semibold text-sm truncate text-foreground">
                                   ...
                                   <p className="text-[10px] text-muted-foreground font-medium truncate">{chapter.lessons.length} bài học</p>
                               </div>
                           </Button>
                          </CollapsibleTrigger>
                          <CollapsibleContent className="mt-3 pl-6 border-l-2 space-y-3">
                           {chapter.lessons.map((lesson)=>(
                               <LessonItem completed={lesson.lessonProgress.find((progress)=>progress.lessonId === lesson.id)?.completed || false} key={lesson.id} lesson={lesson } slug={course.slug} isActive={currentLessonId === lesson.id}></LessonItem>
                           ))}
                          </CollapsibleContent>
                    </Collapsible>
                ))}
